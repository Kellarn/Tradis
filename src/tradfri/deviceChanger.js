...
async function device_changer(argumentArray) {
  let argv = argumentArray;
  ...
  while (position < argv.length) {
    switch (argv[position]) {
      case '--on':
        ...
        accessory.turnOn();
        return 'Turning' + currentDevice.instanceId + 'on';
        break;
      case '--off':
        console.log('Turning', currentDevice.instanceId, 'off');
        accessory.turnOff();
        return 'Turning' + currentDevice.instanceId + 'off';
        break;
      case '--toggle':
        accessory.toggle();
      ...
      case '--blink':
        position++;
        console.log('Blinking device', currentDevice.instanceId);
        // await accessory.toggle();
        // await accessory.toggle();
        setTimeout(() => accessory.turnOn(), 0);
        setTimeout(() => accessory.turnOff(), 1000);
        setTimeout(() => accessory.turnOn(), 2000);
        setTimeout(() => accessory.turnOff(), 3000);
        break;
      default:
